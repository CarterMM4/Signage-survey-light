...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function newProject(name){
    return { id:id(), name:name||'Untitled', createdAt:Date.now(), updatedAt:Date.now(), pages:[], settings:{ colorsByType:SIGN_TYPES, fieldMode:false } };
  }
  ...
  function makePin(x_pct,y_pct){
    return { id:id(), sign_type:'', custom_label:'', room_number:'', room_name:'', building: inputBuilding.value||projectContext.building||'', level: inputLevel.value||projectContext.level||'', x_pct:x_pct, y_pct:y_pct, notes:'', photos:[], lastEdited:Date.now() };
  }
  ...
  function renderProjectsList(){
    ...
    projects
      ...
      .forEach(p=>{
        ...
        el.className='proj'+(p.id===project.id?' active':'');
        ...
        open.onclick=(e)=>{ e.stopPropagation(); selectProject(p.id); };
        el.appendChild(name); el.appendChild(open);
  ...
  function renderProjectLabel(){
    const pinsTotal = project.pages.reduce((a,pg)=>a+((pg.pins&&pg.pins.length)||0),0);
    projectLabel.textContent = `Project: ${project.name} • Pages: ${project.pages.length} • Pins: ${pinsTotal}`;
  }
  ...
  function pinLabel(p){
    if(p.custom_label) return p.custom_label.toUpperCase();
    if(p.sign_type) return (SHORT[p.sign_type]||p.sign_type.slice(0,3).toUpperCase());
    return 'PIN';
    }
  ...
  function renderPins(){
    ...
    (pg.pins||[]).forEach(p=>{
      ...
      el.style.padding = fieldMode? '.32rem .6rem' : '.25rem .5rem';
      el.style.fontSize= fieldMode? '0.95rem' : '0.85rem';
      pinLayer.appendChild(el);

      // Dragging
      el.addEventListener('pointerdown',(e)=>{
        dragging = { el:el, id:p.id };
  ...
  function renderPinsList(){
    ...
    (pg.pins||[]).forEach(p=>{
      ...
      const cb=document.createElement('input'); cb.type='checkbox'; cb.dataset.id=p.id;
      const txt=document.createElement('div');
      txt.innerHTML=`<strong>${p.custom_label||p.sign_type||'-'}</strong> • ${p.room_number||'-'} ${p.room_name||''} <span class="muted">[${p.building||'-'}/${p.level||'-'}]</span>`;
      const go=document.createElement('button'); go.textContent='Go'; go.onclick=(e)=>{ e.stopPropagation(); selectPin(p.id); };
      row.appendChild(cb); row.appendChild(txt); row.appendChild(go);
      row.onclick=()=> selectPin(p.id);
  ...
  function updatePinDetails(){
    ...
    fieldCustom.value=p?.custom_label||'';
  ...
  function parseLenToFeet(input){
    if(!input) return null;
    const s=input.trim().toLowerCase();
    if(s.endsWith('in')){ const n=parseFloat(s); return isNaN(n)?null:(n/12); }
    if(s.endsWith('ft')){ const n=parseFloat(s); return isNaN(n)?null:n; }
    const n=parseFloat(s);
    return isNaN(n)?null:n; // assume feet if unitless
  }
  ...
  on(stage,'click',(e)=>{
    ...
    if(measureMode){
      ...
      const pxDist = dist(measureFirst, pt);
      const entered = prompt('Enter measured length (e.g., "7.5 ft" or "90 in")','');
      const feet = parseLenToFeet(entered||'');
      const m={id:id(), kind:'main', points:[measureFirst, pt]};
      if(feet!=null){ m.userFeet = feet; }
      if(page.scalePxPerFt){ m.feetFromScale = (pxDist/page.scalePxPerFt); }
  ...
  function drawMeasurements(){
    ...
    page.measurements.forEach(m=>{
      ...
      let label='';
      if(typeof m.userFeet==='number') label = m.userFeet.toFixed(2)+' ft';
      else if(page.scalePxPerFt){ const ft=(dist(m.points[0],m.points[1])/page.scalePxPerFt).toFixed(2); label = ft+' ft'; }
      else label = Math.round(dist(m.points[0],m.points[1]))+' px';
  ...
  function showPhoto(){
    const ph=photoState.pin.photos[photoState.idx];
    photoImg.src=ph.dataUrl; photoPinId.textContent=photoState.pin.id; photoName.textContent=ph.name||('Photo '+(photoState.idx+1));
  ...
  function drawPhotoMeasurements(){
    ...
    ph.measurements.forEach(m=>{
      ...
      let label='';
      if(typeof m.userFeet==='number') label = m.userFeet.toFixed(2)+' ft';
      else if(ph.scalePxPerFt){ const ft=(dist(a,b)/ph.scalePxPerFt).toFixed(2); label = ft+' ft'; }
      else label = Math.round(dist(a,b))+' px';
      text.setAttribute('x',midx); text.setAttribute('y',midy-6);
      text.textContent=label;
  ...
  on($('photoMeasureSvg'),'click',(e)=>{
    ...
    if(photoState.calib && !photoState.measureTmp){
      ...
      const raw = prompt('Enter feet for calibration (e.g., 10 or "90 in")','10');
      const ft = parseLenToFeet(raw||'10') || 10;
      ph.scalePxPerFt = px/ft; photoState.calib=null; drawPhotoMeasurements(); saveProject(project);
      return;
    }
  });
  ...
  on(photoImg,'click',(e)=>{
    ...
    const pxDist = dist(photoMeasureFirst,pt);
    const entered = prompt('Enter measured length (e.g., "7.5 ft" or "90 in")','');
    const feet = parseLenToFeet(entered||'');
    const m={id:id(), kind:'photo', points:[photoMeasureFirst, pt]};
    if(feet!=null) m.userFeet=feet;
    if(ph.scalePxPerFt) m.feetFromScale = (pxDist/ph.scalePxPerFt);
  ...
  on($('btnAddPin'),'click',()=>{
    addingPin = !addingPin;
    $('btnAddPin').classList.toggle('pill-btn', addingPin);
  });
  ...
  // Right-panel field edits
  [fieldType, fieldCustom, fieldRoomNum, fieldRoomName, fieldBuilding, fieldLevel, fieldNotes].forEach(el=> on(el,'input',()=>{
    const pin = selectedPin(); if(!pin) return; commit();
    if(el===fieldType) pin.sign_type = el.value || '';
    if(el===fieldCustom) pin.custom_label = el.value || '';
  ...
  on($('btnDuplicate'),'click',()=>{
    ...
    p.id=id(); p.x_pct=Math.min(100, (p.x_pct||0)+2); p.y_pct=Math.min(100, (p.y_pct||0)+2);
    p.lastEdited=Date.now(); currentPage().pins.push(p); saveProject(project); renderPins(); renderPinsList(); selectPin(p.id);
  });
  on($('btnDelete'),'click',()=>{
    const pin=selectedPin(); if(!pin) return;
    if(!confirm('Delete selected pin?')) return;
    commit(); currentPage().pins=currentPage().pins.filter(x=>x.id!==pin.id); saveProject(project); renderPins(); renderPinsList(); project._sel=null; updatePinDetails();
  });
  ...
  on($('stage'),'pointerdown',(e)=>{
    ...
    if(!addingPin) return;
    const pos = toPctCoords(e);
    commit();
    const p = makePin(pos.x_pct,pos.y_pct);
    currentPage().pins.push(p); saveProject(project); renderPins(); renderPinsList(); selectPin(p.id);
    addingPin=false; $('btnAddPin').classList.remove('pill-btn');
  });

  on(pinLayer,'pointermove',(e)=>{
    if(!dragging) return; e.preventDefault();
    const pin = findPin(dragging.id); if(!pin) return;
    const {x_pct,y_pct} = toPctCoords(e);
    dragging.el.style.left = x_pct+'%';
    dragging.el.style.top = y_pct+'%';
    posLabel.textContent = pctLabel(x_pct,y_pct);
  });
  on(pinLayer,'pointerup',(e)=>{
    if(!dragging) return;
    const pin = findPin(dragging.id);
    if(pin){
      commit();
      const {x_pct,y_pct}=toPctCoords(e);
      pin.x_pct=x_pct; pin.y_pct=y_pct; pin.lastEdited=Date.now();
      saveProject(project); renderPinsList(); updatePinDetails();
    }
    dragging.el.releasePointerCapture?.(e.pointerId);
    dragging=null;
  });

  // Resize observer for measure layer
  ...
  window.addEventListener('keydown',(e)=>{
    ...
    if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key)){
      ...
      commit(); const delta = project.settings.fieldMode? 0.5 : 0.2;
   ...
   * Boot         *
   ****************/
  // Populate projects / open last
  projects = loadProjects();
  const last = localStorage.getItem('survey:lastOpenProjectId');
  if(projects.length===0){
    project = newProject('Untitled Project'); saveProject(project);
  }else{
    project = (last && projects.find(p=>p.id===last)) || projects[0];
  }
